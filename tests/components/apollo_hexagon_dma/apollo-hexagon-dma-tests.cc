...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueLoadExecutableDispatchesCnnSlot)
{
    ...
    write_packet(cmdq_base + 2 * CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_LOAD_CODE, slot, APKO_CODE_MAGIC, APKO_CODE_VERSION,
                   0, APKO_CODE_PROGRAM_WORDS,
                   APKO_CODE_OP_MODEL_DISPATCH | CMDQ_DISPATCH_KIND_CNN,
                   APKO_CODE_OP_END });
    write_packet(cmdq_base + 3 * CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_DISPATCH, CMDQ_DISPATCH_EXEC_SLOT_FLAG | slot,
                   input_addr, 0, output_addr, 0, input_bytes, output_bytes });
...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueLoadExecutableDispatchesMnistSlot)
{
    constexpr uint32_t cmdq_base = 0x100;
    constexpr uint32_t input_addr = 0x300;
    constexpr uint32_t output_addr = 0x1200;
    ...
    write_packet(cmdq_base,
                 { CMDQ_OPCODE_LOAD_EXECUTABLE, slot, APKO_MAGIC, APKO_ABI_VERSION,
                   EXEC_FORMAT_APKO_V0, CMDQ_DISPATCH_KIND_MNIST, input_bytes, output_bytes });
    write_packet(cmdq_base + CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_LOAD_PAYLOAD, slot, APKO_PAYLOAD_MAGIC, APKO_PAYLOAD_VERSION,
    ...
    write_packet(cmdq_base + 2 * CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_LOAD_CODE, slot, APKO_CODE_MAGIC, APKO_CODE_VERSION,
                   0, APKO_CODE_PROGRAM_WORDS,
                   APKO_CODE_OP_MODEL_DISPATCH | CMDQ_DISPATCH_KIND_MNIST,
                   APKO_CODE_OP_END });
    write_packet(cmdq_base + 3 * CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_DISPATCH, CMDQ_DISPATCH_EXEC_SLOT_FLAG | slot,
                   input_addr, 0, output_addr, 0, input_bytes, output_bytes });
...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueExecutableSlotRequiresPayload)
{
    constexpr uint32_t cmdq_base = 0x100;
    constexpr uint32_t input_addr = 0x300;
    constexpr uint32_t output_addr = 0x380;
...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueLoadPayloadRejectsBadOpcode)
{
    constexpr uint32_t cmdq_base = 0x100;
    constexpr uint32_t input_bytes = 8 * sizeof(uint32_t);
    constexpr uint32_t output_bytes = 4 * sizeof(uint32_t);
...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueLoadPayloadRejectsMissingCodeWords)
{
    constexpr uint32_t cmdq_base = 0x100;
    constexpr uint32_t input_bytes = 8 * sizeof(uint32_t);
    constexpr uint32_t output_bytes = 4 * sizeof(uint32_t);
...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueLoadCodeRejectsUnsupportedInstruction)
{
    constexpr uint32_t cmdq_base = 0x100;
    constexpr uint32_t input_bytes = 8 * sizeof(uint32_t);
    constexpr uint32_t output_bytes = 4 * sizeof(uint32_t);
    ...
    write_packet(cmdq_base + 2 * CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_LOAD_CODE, slot, APKO_CODE_MAGIC, APKO_CODE_VERSION,
                   0, APKO_CODE_PROGRAM_WORDS,
                   0x00030000u | CMDQ_DISPATCH_KIND_VADD,
                   APKO_CODE_OP_END });
...
TEST_BENCH(ApolloHexagonDmaTestBench, CommandQueueLoadCodeRejectsMissingEnd)
{
    constexpr uint32_t cmdq_base = 0x100;
    constexpr uint32_t input_bytes = 8 * sizeof(uint32_t);
    constexpr uint32_t output_bytes = 4 * sizeof(uint32_t);
    ...
    write_packet(cmdq_base + 2 * CMDQ_PACKET_BYTES,
                 { CMDQ_OPCODE_LOAD_CODE, slot, APKO_CODE_MAGIC, APKO_CODE_VERSION,
                   0, APKO_CODE_PROGRAM_WORDS,
                   APKO_CODE_OP_MODEL_DISPATCH | CMDQ_DISPATCH_KIND_VADD, 0 });
